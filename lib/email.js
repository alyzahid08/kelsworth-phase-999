...
function orderEmailHTML(order, items) {
  const rows = items
    .map(
      (i) => `
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #d8d0c2">${i.product_name} (Unstitched length) × ${i.qty}</td>
        <td style="padding:8px 0;border-bottom:1px solid #d8d0c2;text-align:right">${formatPKR(i.unit_price * i.qty)}</td>
      </tr>`
    )
    .join("");

  return `
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#111111">
    <h2 style="letter-spacing:0.02em">AL-QUTUZ</h2>
    <p>Hi ${order.first_name}, thanks for your order — here's your confirmation.</p>
    <p style="font-family:monospace;background:#f4f0e7;padding:10px 14px;display:inline-block">Order #${order.order_number}</p>
    <table style="width:100%;border-collapse:collapse;margin-top:16px">
      ...
      <tr><td style="padding:10px 0;font-weight:bold;border-top:1px solid #111111">Total</td><td style="text-align:right;font-weight:bold;border-top:1px solid #111111">${formatPKR(order.total)}</td></tr>
    </table>
...
async function sendOrderConfirmation(order, items) {
  ...
  try {
    await getTransporter().sendMail({
      from: process.env.EMAIL_FROM || `AL-QUTUZ <${process.env.SMTP_USER}>`,
      to: order.email,
      subject: `Your AL-QUTUZ order ${order.order_number} is confirmed`,
...
async function sendOrderStatusUpdate(order) {
  ...
  try {
    await getTransporter().sendMail({
      from: process.env.EMAIL_FROM || `AL-QUTUZ <${process.env.SMTP_USER}>`,
      to: order.email,
      subject: `Your AL-QUTUZ order ${order.order_number} is ${copy.subject}`,
...
function welcomeEmailHTML(customer) {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#111111">
    <h2 style="letter-spacing:0.02em">AL-QUTUZ</h2>
    <p>Hi ${customer.firstName}, welcome to AL-QUTUZ.</p>
...
function contactNotificationHTML(msg) {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#111111">
    ...
    <p style="white-space:pre-wrap;background:#f4f0e7;padding:12px 14px;margin-top:10px">${msg.message}</p>
  </div>`;
}
...
function abandonedCartEmailHTML(cart) {
  ...
  const itemRows = items
    ...
    .map((i) => `
      <tr>
        <td style="padding:6px 0;border-bottom:1px solid #d8d0c2">${i.name || i.productName || 'Item'}</td>
        <td style="padding:6px 0;border-bottom:1px solid #d8d0c2">Unstitched length</td>
        <td style="padding:6px 0;border-bottom:1px solid #d8d0c2;text-align:center">${i.qty || 1}</td>
        <td style="padding:6px 0;border-bottom:1px solid #d8d0c2;text-align:right">${formatPKR(i.price || i.unitPrice || 0)}</td>
      </tr>`)
    .join("");
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#111111">
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:16px">
      <tr style="font-size:11px;text-transform:uppercase;color:#55555f;letter-spacing:0.04em">
        ...
        <th style="text-align:left;padding-bottom:8px">Product type</th>
    ...
    <a href="${process.env.SITE_ORIGIN || 'https://www.alqutuz.com'}/collection.html"
       style="display:inline-block;margin-top:16px;padding:12px 28px;background:#111111;color:#fff;text-decoration:none;font-weight:bold;letter-spacing:0.02em">
      COMPLETE YOUR ORDER
    </a>
    <p style="margin-top:24px;color:#4d4d49;font-size:13px">Free shipping on orders above Rs. 5,000. Explore the latest Wash & Wear and Cotton lengths at AL-QUTUZ.</p>
  </div>`;
}
...
async function sendAbandonedCartEmail(cart) {
  ...
  try {
    await getTransporter().sendMail({
      from: process.env.EMAIL_FROM || `AL-QUTUZ <${process.env.SMTP_USER}>`,
      to: email,
      subject: "You left something behind at AL-QUTUZ",

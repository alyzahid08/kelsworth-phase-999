...
function orderEmailHTML(order, items) {
  const rows = items
    .map(
      (i) => `
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #d8d0c2">${i.product_name} (Size ${i.size}) × ${i.qty}</td>
  ...
  const itemRows = items
    ...
    .map((i) => `
      <tr>
        ...
        <td style="padding:6px 0;border-bottom:1px solid #d8d0c2">${i.size || ''}</td>

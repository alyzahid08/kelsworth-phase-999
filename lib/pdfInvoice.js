...
const LIGHT_COLOR = "#55524a";
const ACCENT_COLOR = "#b89445";
...
async function generateInvoicePDF(order, items) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        Title: `Invoice ${order.order_number} — AL-QUTUZ`,
        Author: "AL-QUTUZ",
    ...
    // ---- Header ----
    doc.font(FONT_BOLD).fontSize(28).fillColor(BRAND_COLOR).text("AL-QUTUZ", { align: "left" });
    doc.moveDown(0.2);
    doc.font(FONT_REGULAR).fontSize(9).fillColor(LIGHT_COLOR).text("Premium Men's Unstitched Fabric", { align: "left" });
    ...
    const headers = ["Item", "", "Product type", "Qty", "Unit Price", "Total"];
    ...
    for (const item of items) {
      ...
      doc.text("Unstitched length", colX[2], rowY);
    ...
    doc.text("Thank you for shopping with AL-QUTUZ.", 50, doc.y, { align: "center", width: pageWidth });
    doc.text("Free exchange within 14 days of delivery, tags attached and unworn.", { align: "center", width: pageWidth });
    doc.text("For support, reply to this email or visit alqutuz.com/contact.html.", { align: "center", width: pageWidth });
